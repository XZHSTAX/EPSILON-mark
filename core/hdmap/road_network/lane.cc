...
#include "../../common/inc/common/utils/math.h"
#include "lane_map.h"
#include "../utils/admap_convertion.h"
...
Lane::Lane(const ::ad::map::lane::Lane::ConstPtr admap_lane)
  ...
  // 道路初始化时，就更新了lane对象的waypoint
  // ATTENTION
  CreateWayPoints();
}

// 通过递归的方式，找到所有相邻车道，并且存入neighbor_lanes_中。注意，会把所有相邻车道存入
// 比如 4 | 3 | 2 | 1 ，1号是当前车道，2号已经被存入了那么3,4车道都会被存入neighbor_lanes_中
...
/**
 * 输入周边车道的ID，类型（左边车道还是右边车道）
 * 把该车道的信息存入neighbor_lanes_
 * 即根据旁边车道ID和类型，更新本车道的neighbor_lanes_
 */
...
// ATTENTION: 在此处更新了lane对象的waypoint
...
// 根据type(左车道和右车道)，返回对应的车道，返回类型为Lane
